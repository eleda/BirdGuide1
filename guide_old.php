		...
		while ( (! feof ( $file )) && ($y == false) ) {
			...
			if (strlen ( $mc ) >= strlen ( $latinword )) {
				if (substr ( $mc, 0, strlen ( $latinword ) + 1 ) == ($latinword . "=")) {
					$kin = substr ( $mc, strlen ( $latinword ) + 1 );
					$y = true;
				}
			}
		}
...
function getinfo($fi, $tag) {
	...
	while ( (! feof ( $file )) && ($y == false) ) {
		$mc = fgets ( $file );
		$mc = substr ( $mc, 0, strlen ( $mc ) - 2 );
		if (substr ( $mc, 0, strlen ( $tag ) + 1 ) == $tag . "=") {
			$kin = substr ( $mc, strlen ( $tag ) + 1 );
			$y = true;
		}
	}
	fclose ( $file );
	...
	while ( (($fil = readdir ( $dir )) !== false) && ($y == false) ) {
		...
		if ((substr ( $fil, strlen ( $fil ) - 4 ) == ".spe") && ($fil != ".") && ($fil != "..")) {
			...
			while ( ! feof ( $file ) ) {
				...
				$mc = substr ( $mc, 0, strlen ( $mc ) - 2 );
				// echo $mc;
				if (substr ( $mc, 0, 7 ) == "regnum=") {
					$reg = substr ( $mc, 7 );
				}
				if (substr ( $mc, 0, 7 ) == "phylum=") {
					$phy = substr ( $mc, 7 );
				}
				if (substr ( $mc, 0, 10 ) == "subphylum=") {
					$sub = substr ( $mc, 10 );
				}
				...
				if (substr ( $mc, 0, 8 ) == "familia=") {
					$fam = substr ( $mc, 8 );
				}
				if (substr ( $mc, 0, 6 ) == "genus=") {
					$gen = substr ( $mc, 6 );
				}
			...
			if (($gen == $sp ["genus"]) && ($spe == $sp ["species"])) {
...
function getspeciesdetails($fil, &$sp) {
	if ($fil != "") {
		...
		while ( ! feof ( $file ) ) {
			$mc = fgets ( $file );
			// echo $mc;
			if (substr ( $mc, 0, 5 ) == "HU_n=") {
				$sp ["n"] = substr ( $mc, 5 );
			}
			if (substr ( $mc, 0, 5 ) == "HU_d=") {
				$sp ["d"] = substr ( $mc, 5 );
			}
			if (substr ( $mc, 0, 5 ) == "HU_s=") {
				$sp ["s"] = substr ( $mc, 5 );
			}
			if (substr ( $mc, 0, 5 ) == "HU_p=") {
				$sp ["p"] = substr ( $mc, 5 );
			}
		}
		fclose ( $file );
	...
	while ( ($fil = readdir ( $dir )) !== false ) {
		...
		if (strtolower ( substr ( $fil, strlen ( $fil ) - 4 ) == ".spe" ) && ($fil != ".") && ($fil != "..")) {
			...
			while ( ! feof ( $file ) ) {
				...
				$mc = substr ( $mc, 0, strlen ( $mc ) - 2 );
				$mc2 = strtolower ( $mc );
				if (substr ( $mc2, 0, 7 ) == "regnum=") {
					$reg = substr ( $mc, 7 );
				}
				if (substr ( $mc2, 0, 7 ) == "phylum=") {
					$phy = substr ( $mc, 7 );
				}
				if (substr ( $mc2, 0, 10 ) == "subphylum=") {
					$sub = substr ( $mc, 10 );
				}
				if (substr ( $mc2, 0, 8 ) == "classis=") {
					$cla = substr ( $mc, 8 );
				}
			}
			fclose ( $file );
			...
			$exi = false;
			for($i = 0; $i < count ( $phlyums ); $i ++) {
				if ($phlyums [$i] == $rootphy) {
					$exi = 1;
				}
			}
			if ($exi == 0) {
				array_push ( $phlyums, $rootphy );
				array_push ( $shtphlyums, $phy );
			}
			
			$exi = false;
			for($i = 0; $i < count ( $subphlyums ); $i ++) {
				if ($subphlyums [$i] == $rootsub) {
					$exi = 1;
				}
			}
			if ($exi == 0) {
				array_push ( $subphlyums, $rootsub );
				array_push ( $shtsubphlyums, $sub );
			}
		...
		while ( ($fil = readdir ( $dir )) !== false ) {
			...
			if (strtolower ( substr ( $fil, strlen ( $fil ) - 4 ) == ".spe" ) && ($fil != ".") && ($fil != "..")) {
				...
				while ( ! feof ( $file ) ) {
					...
					if (substr ( $mc2, 0, 5 ) == "ordo=") {
						$ord = substr ( $mc, 5 );
					}
					if (substr ( $mc2, 0, 8 ) == "familia=") {
						$fam = substr ( $mc, 8 );
					}
					if (substr ( $mc2, 0, 6 ) == "genus=") {
						$gen = substr ( $mc, 6 );
					}
					if (substr ( $mc2, 0, 8 ) == "species=") {
						$spe = substr ( $mc, 8 );
					}
					if (substr ( $mc2, 0, 5 ) == "hu_n=") {
						$hun = substr ( $mc, 5 );
					}
				}
				fclose ( $file );
				...
				if (($posgen !== false) || ($posspe !== false) || ($poshun !== false)) {
					echo "<li>" . "<a href='" . curpurl () . "?view=species&genus=" . $gen . "&species=" . $spe . "'>" . "<b>" . $hun . "</b> " . $gen . " " . $spe . "</a></br>Rend: " . $ord . ",  Család: " . $fam . "</li>";
					$fnd ++;
				}
			}
		}
...
function showspecies(&$sp) {
	...
	echo "  <tr>";
	echo "    <td width='463' valign='top'><p>" . $sp ["d"] . "</td></p>";
	...
	echo "<a href=" . $imalink . " target='_blank'><img src=" . $speima . " alt='picture of species' width='320' height='240' /></a></div></td>";
	echo "  </tr>";
	echo "  <tr>";
	echo "    <td colspan='2'>";
	...
	if (is_dir ( $pa )) {
		$dir = opendir ( $pa );
		while ( ($fil = readdir ( $dir )) !== false ) {
			if ((substr ( $fil, strlen ( $fil ) - 4 ) == ".mp3") && ($fil != ".") && ($fil != "..")) {
				array_push ( $snds, $fil );
			}
		}
	}
	...
	if (count ( $snds ) > 0) {
		...
		for($i = 0; $i < count ( $snds ); $i ++) {
			...
			echo "<li><a href=" . $pat . " onClick=" . chr ( 34 ) . "return popup(this, 'Video')" . chr ( 34 ) . ">" . $snds [$i] . "</a></li>";
		}
		echo "</ul>";
	}
	...
	while ( ($fil = readdir ( $dir )) !== false ) {
		...
		if ((substr ( $fil, strlen ( $fil ) - 4 ) == ".spe") && ($fil != ".") && ($fil != "..")) {
			...
			while ( ! feof ( $file ) ) {
				$mc = fgets ( $file );
				$mc = substr ( $mc, 0, strlen ( $mc ) - 2 );
				if (substr ( $mc, 0, strlen ( $ty ) + 1 ) == $ty . "=") {
					$kin = substr ( $mc, strlen ( $ty ) + 1 );
				}
			}
			fclose ( $file );
...
function showds($ty, $con) {
	switch ($ty) {
		case "classis" :
			...
			echo " &gt;&gt; " . tw ( $reg, "hu" ) . " (" . $reg . ") &gt;&gt; " . tw ( $phy, "hu" ) . " (" . $phy . ") &gt;&gt; " . tw ( $sub, "hu" ) . " (" . $sub . ")</p>";
